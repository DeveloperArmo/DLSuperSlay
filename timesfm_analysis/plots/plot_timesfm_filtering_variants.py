...
SOURCE_DIR = REPO_ROOT / "Google Time-Series Foundation Model"
OUTPUT_DIR = REPO_ROOT / "timesfm_analysis" / "plots"
...
VARIANTS = [
    VariantSpec("TimesFM_SSMI_Metrics.npz", "Baseline TimesFM"),
    VariantSpec("TimesFM_SSMI_Filtered_Metrics.npz", "Moving Average"),
    VariantSpec("TimesFM_SSMI_HP_Metrics.npz", "Hodrick-Prescott"),
    VariantSpec("TimesFM_SSMI_Kalman_Metrics.npz", "Kalman"),
    VariantSpec("TimesFM_SSMI_Butterworth_Metrics.npz", "Butterworth"),
]
...
def render_directional_accuracy_summary_chart(
    ...
    *,
    ...
    ax.set_ylabel("TimesFM Variant", fontsize=14, fontweight="semibold")
...
def main() -> None:
    ...
    generated_paths: Iterable[Path] = [
        render_boxplot(
            r2_distributions,
            title="TimesFM Frequency-Filtering Variants: R^2 Distribution",
            ylabel="R^2",
            output_name="timesfm_r2_distribution.png",
            ylim=(-0.02, 1.02),
        ),
        render_boxplot(
            mape_distributions,
            title="TimesFM Frequency-Filtering Variants: MAPE Distribution",
            ylabel="MAPE (%)",
            output_name="timesfm_mape_distribution.png",
        ),
        render_boxplot(
            rmse_distributions,
            title="TimesFM Frequency-Filtering Variants: RMSE Distribution",
            ylabel="RMSE",
            output_name="timesfm_rmse_distribution.png",
        ),
        render_directional_accuracy_summary_chart(
            directional_accuracy_rows,
            title="TimesFM Frequency-Filtering Variants: Directional Accuracy",
            output_name="timesfm_directional_accuracy_histogram.png",
        ),
    ]
