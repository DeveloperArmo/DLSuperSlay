"""Fine-tune TWO TimesFM models on Kalman-decomposed SSMI train set — Kalman v3.

Mirrors finetune_timesfm_decomposed_v3.py, swapping the HP filter for the
scalar Kalman filter that won the Hyperparams sweep
(process_noise=0.1, measurement_noise=1.0).

v3 setup (same as the HP-based v3):
  - Trainable: input_ff + horizon_ff + last 4 transformer blocks (16-19) — ~23%
  - Per-window z-score normalization on raw context
  - AdamW, lr=3e-5, weight_decay=0.01
  - Grad clipping (norm 1.0)
  - Early stopping (max 5 epochs, patience 2)
  - Batch size 4, 90/10 chronological train/val split

Decomposition (matches decompose_kalman in sim_Hyperparams.ipynb):
  - One-pass scalar Kalman filter (causal, no look-ahead)
  - q = 0.1, r = 1.0
  - low  = filtered state estimate
  - high = y_train - low

Train range: 2001-10-01 -> 2017-12-31 (from trimmed SSMI)
Saves checkpoints next to this script:
  - timesfm_ssmi_low_kalman_v3.pt
  - timesfm_ssmi_high_kalman_v3.pt
"""
...
CHECKPOINT_LOW = SCRIPT_DIR / "timesfm_ssmi_low_kalman_v3.pt"
CHECKPOINT_HIGH = SCRIPT_DIR / "timesfm_ssmi_high_kalman_v3.pt"
...
class ComponentWindows(Dataset):
    """Sliding windows with per-window z-score normalization (same as v2/HP v3)."""
...
def kalman_decompose_full(
    ...
    """One-pass causal scalar Kalman filter — identical to the sweep's decompose_kalman."""
...
def finetune_component(name: str, series: np.ndarray, ckpt_path: Path) -> None:
    print(f"\n{'=' * 60}\nFine-tuning model_{name} (kalman v3)\n{'=' * 60}")
