"""Fine-tune TWO TimesFM models (low-pass / high-pass) — v2.

Improvements over v1:
  1. UNFREEZE MORE LAYERS: input_ff_layer + horizon_ff_layer + last 2 transformer
     blocks (layers 18, 19). ~17% of params trainable vs. 2.4% in v1.
...
Saves checkpoints next to this script:
  - timesfm_ssmi_low_v2.pt
  - timesfm_ssmi_high_v2.pt
"""
...
# Unfreeze prefixes: params whose names START with any of these are trainable.
TRAINABLE_PREFIXES = (
    "input_ff_layer.",
    "horizon_ff_layer.",
...
CHECKPOINT_LOW = SCRIPT_DIR / "timesfm_ssmi_low_v2.pt"
CHECKPOINT_HIGH = SCRIPT_DIR / "timesfm_ssmi_high_v2.pt"
...
class ComponentWindows(Dataset):
    """Sliding windows with per-window z-score normalization.
    ...
    def __getitem__(self, i: int):
        ...
        if ctx_std < 1e-6:
            ctx_std = 1.0  # degenerate window; don't divide by ~0
...
def freeze_with_prefixes(model: nn.Module, trainable_prefixes: tuple[str, ...]) -> tuple[int, int, list[str]]:
    """Unfreeze params whose name starts with ANY prefix in the list. Freeze rest."""
    trainable, total = 0, 0
...
def horizon_predictions(out: torch.Tensor, hor: int) -> torch.Tensor:
    """Extract the 30-step forecast from TimesFM's 4D output."""
    return out[:, -1, :hor, 0]
