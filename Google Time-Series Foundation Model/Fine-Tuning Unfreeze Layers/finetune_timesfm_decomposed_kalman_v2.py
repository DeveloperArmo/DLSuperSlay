"""Fine-tune TWO TimesFM models on Kalman-decomposed SSMI train set — Kalman v2.

Mirrors finetune_timesfm_decomposed_v2.py, swapping the HP filter for the
scalar Kalman filter that won the Hyperparams sweep
(process_noise=0.1, measurement_noise=1.0).

v2 setup (same as the HP-based v2):
  - Trainable: input_ff_layer + horizon_ff_layer + last 2 transformer blocks
    (layers 18, 19) — ~13% of params
  - Per-window z-score normalization on raw context
  - AdamW, lr=5e-5, 3 epochs, MSE loss
...
Saves checkpoints next to this script:
  - timesfm_ssmi_low_kalman_v2.pt
  - timesfm_ssmi_high_kalman_v2.pt
"""
...
CHECKPOINT_LOW = SCRIPT_DIR / "timesfm_ssmi_low_kalman_v2.pt"
CHECKPOINT_HIGH = SCRIPT_DIR / "timesfm_ssmi_high_kalman_v2.pt"
...
class ComponentWindows(Dataset):
    """Sliding windows with per-window z-score normalization (same as HP v2)."""
...
def finetune_component(name: str, series: np.ndarray, ckpt_path: Path) -> None:
    print(f"\n{'=' * 60}\nFine-tuning model_{name} (kalman v2)\n{'=' * 60}")
